...
int audio_backend_run(Engine* engine, int seconds)
{
    ...
    // DEBUG: list all output devices (helps diagnose Windows/MSYS2 device quirks)
    ...
    for (PaDeviceIndex i = 0; i < ndev; ++i) {
        ...
        fprintf(stderr, "  [%d] %s (maxOutCh=%d)\n",
                (int)i, di->name, di->maxOutputChannels);
    }
    // DEBUG END

    // NOTE:
    // We intentionally use Pa_OpenDefaultStream() here.
    // - Original intent: "just play sound" by relying on the OS default output device.
    // - We previously tried explicit device selection via Pa_OpenStream() + PaStreamParameters,
    //   but on MSYS2/Windows, PortAudio device indices can be Host-API-specific.
    //   As a result, a device index that looks correct in the global list may still fail
    //   with "Invalid device" under the active Host API.
    //
    // Practical workflow for now:
    // 1) Set Windows "Default output device" to your intended speakers (e.g. Realtek).
    // 2) Let Pa_OpenDefaultStream() follow that OS default reliably.
